...
function vrniHtmlZastavico(trenutnaStranka, povratniKlic) {
    vrniDrzavoStranke(trenutnaStranka, (drzava) => {
        let cca2 = '';
        if (drzava === 'USA') {
            const flagUrl = `https://flagcdn.com/16x12/us.png`;
            const htmlZastavica = `<img src="${flagUrl}" alt="us" width="16" height="12" />`;
            povratniKlic(htmlZastavica);
        }else if (drzava === 'India') {
            const flagUrl = `https://flagcdn.com/16x12/in.png`;
            const htmlZastavica = `<img src="${flagUrl}" alt="in" width="16" height="12" />`;
            povratniKlic(htmlZastavica);
        }else {
            const restCountriesUrl = `https://restcountries.com/v2/name/${drzava}`;
            fetch(restCountriesUrl)
                .then(response => response.json())
                .then(data => {
                    if (data && data.length > 0 && data[0].alpha2Code) {
                        cca2 = data[0].alpha2Code.toLowerCase();
                    }
                    const flagUrl = `https://flagcdn.com/16x12/${cca2}.png`;
                    const htmlZastavica = `<img src="${flagUrl}" alt="${cca2}" width="16" height="12" />`;
                    povratniKlic(htmlZastavica);
                })
                .catch(error => {
...
function izracunajPopust(imeDestinacije) {
        ...
        strankaIzRacuna(racunId, (stranka) => {
            destinacijeIzRacuna(racunId, (napaka, destinacije) => {
                ...
                destinacije.forEach((destinacija, i) => {
                    ...
                    destinacija.popustStopnja = izracunajPopust(destinacija.ime);
        ...
        destinacijeIzKosarice(zahteva, (napaka, destinacije) => {
            if (napaka || !destinacije) {
                ...
                destinacije.forEach((destinacija, i) => {
                    ...
                    destinacija.popustStopnja = izracunajPopust(destinacija.ime); // Izračun popusta glede na ime destinacije
